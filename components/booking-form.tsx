    ...
    <div>
      {label && (
        <div className="flex items-baseline gap-2 mb-3">
          <span className="font-bold text-[var(--brand-primary)] text-sm">{label}</span>
...
export function BookingForm() {
  ...
  const selectedExtrasList  = [...extraOptions, ...accessoryOptions].filter(e => formData.selectedExtras[e.id])
  const addOnsSummary = [
              ...
              return (
                <div key={s.id} className="flex justify-between gap-4 text-sm">
                  <span className="text-gray-400 shrink-0">
                    {s.label} × {qty}
                    {promoActive && <span className="ml-1 text-[var(--brand-accent)] text-[10px] font-bold">PROMO</span>}
                  </span>
                  <span className="font-medium text-[var(--brand-primary)]">${(linePrice / 100).toFixed(2)}</span>
                </div>
              )
            })}
            {discountCents > 0 && (
      ...
      <CardContent className="pt-6">
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            {/* Promo banner */}
            {promoActive && (
              <div className="rounded-2xl bg-[var(--brand-accent)] text-white px-4 py-3 flex items-center gap-3">
                ...
                return (
                  <div key={s.id}
                    className={cn(
                      ...
                      isSelected ? "border-[var(--brand-accent)] bg-[#fdf6f3]" : "border-gray-200 bg-white"
                    )}>
                    {/* Size label + price */}
                    <div className="flex items-start justify-between mb-1">
                      <span className={cn("font-extrabold text-sm", isSelected ? "text-[var(--brand-primary)]" : "text-gray-700")}>{s.label}</span>
                      <div className="text-right">
                        {promoActive ? (
                          <div className="flex items-center gap-1.5 justify-end">
                            <span className="text-xs line-through text-red-400 font-semibold">{origPrice}</span>
                            <span className="font-extrabold text-sm text-[var(--brand-accent)]">${(promoPriceCents / 100).toFixed(0)}</span>
                            {promoSavings && (
                              <span className="text-[9px] bg-green-100 text-green-700 font-bold px-1 py-0.5 rounded">SAVE</span>
                            )}
                          </div>
                        ) : (
                          <span className={cn("font-extrabold text-sm", isSelected ? "text-[var(--brand-accent)]" : "text-gray-400")}>{origPrice}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className={cn("text-2xl font-extrabold tabular-nums min-w-[2rem] text-center",
                        isSelected ? "text-[var(--brand-primary)]" : "text-gray-300")}>
                        {qty}
                      </span>
                      <button type="button"
                        onClick={() => setQty(s.id as SizeId, 1)}
                        className="w-8 h-8 rounded-full border-2 border-[var(--brand-primary)] text-[var(--brand-primary)] font-bold text-lg flex items-center justify-center hover:bg-[var(--brand-primary)] hover:text-white transition-colors">
                        +
                      </button>
                    </div>
                  </div>
            ...
            {totalCount > 0 && (
              <div className="bg-[#fdf6f5] rounded-xl p-4 flex items-center justify-between">
                <div className="text-sm text-[var(--brand-primary)]/60 font-medium">
                  {selectedSizes.map(s => `${quantities[s.id as SizeId]}× ${s.label}`).join(", ")}
                </div>
                <span className="text-2xl font-extrabold text-[var(--brand-accent)]">${(subtotalCents / 100).toFixed(2)}</span>
              </div>
            )}
            ...
            <details className="group">
              <summary className="flex items-center gap-2 text-sm text-gray-400 hover:text-[var(--brand-primary)] transition-colors cursor-pointer list-none">
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              <h3 className="text-xl font-extrabold text-[var(--brand-primary)] mb-1">{tb.customizeWash}</h3>
              <p className="text-sm text-gray-400">{tb.addOnsOptional}</p>
            </div>

            {detergentOptions.length > 0 && (
              <div>
                <h4 className="font-bold text-[var(--brand-primary)] text-sm mb-3">{tf.detergentPreference}</h4>
            ...
            {extraOptions.length > 0 && (
              <div>
                <h4 className="font-bold text-[var(--brand-primary)] text-sm mb-3">{tf.treatmentAddOns}</h4>
                <div className="space-y-2">
            ...
            {accessoryOptions.length > 0 && (
              <div>
                <h4 className="font-bold text-[var(--brand-primary)] text-sm mb-3">{tf.accessoryAddOns}</h4>
                <div className="space-y-2">
        ...
        {step === 3 && (
          <div className="space-y-5">
            ...
            <div className="space-y-4">
                ...
                <div key={key} className="space-y-1.5">
                  ...
                  {key === "email" && emailCheckState === "otp_sent" && (
                    ...
                    <div className="rounded-xl bg-[#f8fafc] border border-gray-100 p-4 space-y-3 mt-1">
                        ...
                        <>
                          ...
                          <div className="flex gap-2">
                            <Input
                              ...
                              className="font-mono text-center text-lg tracking-widest h-11 border-gray-200 focus:border-[var(--brand-accent)] w-36"
                            />
                      ...
                      {authMethod === "phone" && !phoneOtpSent && (
                        <div className="space-y-2">
                          <Input type="tel" placeholder="(407) 555-0100"
                            ...
                            className="h-11 border-gray-200 focus:border-[var(--brand-accent)] text-sm" />
                          ...
                          <div className="flex gap-2">
                            ...
                            <button type="button" onClick={sendPhoneOtp} disabled={phoneOtpLoading}
                              className="flex-[2] h-10 rounded-xl bg-[var(--brand-primary)] hover:bg-[#1a3a5c] disabled:opacity-40 text-white text-sm font-bold transition-colors">
                      ...
                      {authMethod === "phone" && phoneOtpSent && (
                        <div className="space-y-2">
                          <p className="text-sm text-[var(--brand-primary)]">Enter the code sent to <strong>{phoneForAuth}</strong></p>
                          <div className="flex gap-2">
                            ...
                            <button type="button" onClick={verifyPhoneOtp}
                              ...
                              className="flex-1 bg-[var(--brand-primary)] hover:bg-[#1a3a5c] disabled:opacity-40 text-white text-sm font-bold rounded-xl transition-colors">
                      ...
                      {authMethod !== "phone" && (
                        <div className="space-y-2">
                          <button type="button" onClick={handleGoogleAuth}
                            className="w-full flex items-center justify-center gap-2.5 h-11 rounded-xl border border-gray-200 bg-white hover:border-[var(--brand-primary)] transition-colors text-sm font-semibold text-[var(--brand-primary)]">
                          ...
                          <button type="button" onClick={sendEmailOtp} disabled={!formData.email.includes("@")}
                            className="w-full flex items-center justify-center gap-2.5 h-11 rounded-xl border border-gray-200 bg-white hover:border-[var(--brand-primary)] disabled:opacity-40 transition-colors text-sm font-semibold text-[var(--brand-primary)]">
              ...
              {/* Pickup address */}
              <div className="space-y-2">
                <Label className="font-semibold text-[var(--brand-primary)] text-sm">{tf.pickupAddressLabel}</Label>
                <AddressAutocomplete
              ...
              {!formData.sameAddress && (
                <div className="space-y-2">
                  <Label className="font-semibold text-[var(--brand-primary)] text-sm">{tf.deliveryAddressLabel}</Label>
                  <AddressAutocomplete
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button className="flex-[2] h-12 text-sm font-bold bg-[var(--brand-primary)] hover:bg-[#1a3a5c]" disabled={!canStep3} onClick={() => setStep(4)}>
                {tf.continueConfirm}
              </Button>
            </div>
          </div>
        ...
        {step === 4 && (
          <div className="space-y-5">
                ...
                return (
                  <div key={s.id} className="flex justify-between gap-4">
                    <span className="text-gray-400 shrink-0 flex items-center gap-1.5">
                      {s.label} × {qty}
                      {promoActive && <span className="text-[9px] bg-[var(--brand-accent)]/10 text-[var(--brand-accent)] font-bold px-1 rounded">$33 ea</span>}
                    </span>
                    <span className="font-medium text-[var(--brand-primary)]">${(linePrice / 100).toFixed(2)}</span>
                  </div>
                )
            ...
            <details className="group">
              <summary className="flex items-center justify-between cursor-pointer text-sm font-semibold text-[var(--brand-primary)] bg-gray-50 rounded-xl px-4 py-3 hover:bg-[#fdf6f5] transition-colors list-none">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button className="flex-[2] h-12 text-sm font-bold bg-[var(--brand-primary)] hover:bg-[#1a3a5c]" disabled={!canStep4} onClick={() => setStep("payment")}>
                {tf.proceedToPayment}
              </Button>
            </div>
          </div>
