...
import { addAccessWindow, deleteAccessWindow } from "@/app/actions/facility-windows"
...
export function FacilityAccessWindowsEditor({
  facilityId,
  initial,
}: {
  facilityId: string
  initial: AccessWindow[]
}) {
  const [windows, setWindows] = useState<AccessWindow[]>(initial)
  ...
  const [startTime, setStart]   = useState("21:00")
  const [endTime, setEnd]       = useState("06:00")
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await addAccessWindow(fd)
      if (res?.error) { setErr(res.error); return }
      if (res?.window) {
        setWindows(prev => [...prev, res.window])
        setShowForm(false)
        setLabel(""); setDays(new Set()); setStart("21:00"); setEnd("06:00"); setNotes("")
      }
    })
  }
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-100">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-xs font-extrabold text-[#0D2240] uppercase tracking-widest">Our Access Windows</h4>
        <button
          type="button"
      ...
      {windows.length === 0 && !showForm && (
        <p className="text-xs text-gray-400 italic mb-2">No access windows set — facility treated as always available.</p>
      )}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 flex-wrap mt-0.5">
                ...
                <span className="text-xs font-semibold text-[#0D2240]">
                  ...
                  {w.overnight && <span className="ml-1 text-[10px] text-purple-600 font-bold bg-purple-50 px-1.5 py-0.5 rounded-full border border-purple-200">Overnight</span>}
                </span>
              </div>
              {/* Day dots */}
        ...
        <form onSubmit={handleAdd} className="bg-blue-50/60 rounded-xl border border-blue-100 p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={label} onChange={e => setLabel(e.target.value)} placeholder="e.g. Night Shift"
            ...
            <div>
              ...
              <input value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Use side entrance"
                className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30" />
            </div>
          </div>
          ...
          {/* Day picker */}
          <div>
            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Days We Can Access</label>
            <div className="flex gap-1.5 flex-wrap">
              ...
              <button type="button" onClick={() => setDays(new Set([1,2,3,4]))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Access Starts</label>
