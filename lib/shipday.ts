...
export async function createShipdayOrder(booking: ShipdayOrderInput): Promise<ShipdayOrderIds> {
  ...
  const pickupItems = isWashFold
    ? [{ name: "Wash & Fold — Pickup", quantity: booking.num_bags ?? 1, unitPrice: total,
         details: `Wash & Fold pickup — est. ${booking.pounds ?? "?"} lbs, ${booking.num_bags ?? 1} bag(s). Final charge by actual weight.` }]
    : isWashOnly
    ? [{ name: "Wash Only — Pickup", quantity: booking.num_bags ?? 1, unitPrice: total,
         details: `Wash Only pickup — est. ${booking.pounds ?? "?"} lbs, ${booking.num_bags ?? 1} bag(s). Final charge by actual weight.` }]
    : Array.from({ length: booking.num_comforters }, (_, i) => ({
        name: "Comforter Wash — Pickup", quantity: 1,
        unitPrice: (booking.total_amount / booking.num_comforters / 100).toFixed(2),
        details: `Comforter #${i + 1} — Pickup`,
      }))

  const deliveryItems = isWashFold
    ? [{ name: "Wash & Fold — Delivery", quantity: booking.num_bags ?? 1, unitPrice: total,
         details: "Wash & Fold delivery — clean, folded laundry returned to customer." }]
    : isWashOnly
    ? [{ name: "Wash Only — Delivery", quantity: booking.num_bags ?? 1, unitPrice: total,
         details: "Wash Only delivery — clean laundry returned in bag, unfolded." }]
    : Array.from({ length: booking.num_comforters }, (_, i) => ({
        name: "Comforter Wash — Delivery", quantity: 1,
        unitPrice: (booking.total_amount / booking.num_comforters / 100).toFixed(2),
        details: `Comforter #${i + 1} — Delivery`,
      }))
  ...
  const pickupPayload = {
    orderNumber: `${baseCode}P`,
    customerName: booking.customer_name, customerAddress: booking.customer_address,
    customerEmail: booking.customer_email, customerPhoneNumber: booking.customer_phone,
    restaurantName: "WashFold Orlando", restaurantAddress: facilityAddress, restaurantPhoneNumber: facilityPhone,
    pickupDate: toShipdayDate(booking.pickup_date), expectedPickupTime: timeWindowToTime(booking.pickup_time_window),
    paymentMethod: "PAID_ONLINE", subtotal: total, tax: "0.00", totalOrderCost: total, tips: "0.00",
    orderItems: pickupItems,
  }
  ...
  const deliveryPayload = {
    ...
    customerEmail: booking.customer_email, customerPhoneNumber: booking.customer_phone,
    restaurantName: "WashFold Orlando", restaurantAddress: facilityAddress, restaurantPhoneNumber: facilityPhone,
    pickupDate: toShipdayDate(booking.delivery_date), expectedPickupTime: timeWindowToTime(booking.delivery_time_window),
    paymentMethod: "PAID_ONLINE", subtotal: total, tax: "0.00", totalOrderCost: total, tips: "0.00",
    orderItems: deliveryItems,
  }
