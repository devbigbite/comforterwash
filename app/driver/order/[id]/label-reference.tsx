  ...
  if (fullscreen) {
    return (
      ...
      >
        ...
        <p className="text-white/40 text-xs font-bold uppercase tracking-[0.3em] mb-2">Write on all bags</p>
        <p className="text-white font-black font-mono tracking-widest"
          style={{ fontSize: "clamp(3rem, 15vw, 6rem)", lineHeight: 1 }}>
          {orderCode}
        </p>

        <p className="mt-8 text-white/30 text-base font-semibold">
          {bags.length} bag{bags.length !== 1 ? "s" : ""} · same code on every bag
        </p>

        <p className="mt-12 text-white/20 text-xs text-center">
          Write with sharpie · all bags get the same number
        </p>
      </div>
    )
  }
      ...
      <div className="flex items-center gap-4 px-5 py-4">
        <div className="flex-1 min-w-0">
          <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest mb-0.5">
            Write on all {bags.length} bag{bags.length !== 1 ? "s" : ""}
          </p>
          <p className="text-white font-black font-mono text-3xl tracking-widest leading-none">{orderCode}</p>
          <p className="text-white/30 text-xs mt-1 truncate">{customerName} · same code on every bag</p>
        </div>
        <button
