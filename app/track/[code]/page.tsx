...
export default async function TrackPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fb]">
      ...
      {/* Header */}
      <div className="bg-[#0D2240] px-4 pt-8 pb-6 text-center">
        <Link href="/" className="inline-flex items-center gap-2 text-[#E8726A] font-extrabold text-xl mb-4">
        ...
        <div className={`inline-block mt-3 px-4 py-1.5 rounded-full text-sm font-bold
          ${isDone ? "bg-green-500 text-white" : "bg-[#E8726A] text-white"}`}>
                ...
                <div key={i} className="flex gap-3">
                  ...
                  <div className="flex flex-col items-center">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-base shrink-0 z-10
                      ${isCompleted ? "bg-[#0D2240] text-white"
                      : isCurrent   ? "bg-[#E8726A] text-white shadow-lg ring-4 ring-[#E8726A]/20"
                      : "bg-gray-100 text-gray-300"}`}>
                      {isCompleted ? "✓" : step.icon}
                    </div>
                    {!isLast && (
                      <div className={`w-0.5 flex-1 my-1 ${isCompleted ? "bg-[#0D2240]" : "bg-gray-100"}`}
                  ...
                  <div className={`pt-1.5 pb-5 ${isLast ? "pb-0" : ""}`}>
                    <p className={`font-bold text-sm leading-tight
                      ${isCompleted ? "text-[#0D2240]"
                      : isCurrent   ? "text-[#E8726A]"
                      : "text-gray-300"}`}>
                      {step.label}
                      {isCurrent && <span className="ml-2 text-[10px] font-black uppercase tracking-widest bg-[#E8726A] text-white px-2 py-0.5 rounded-full">Now</span>}
                    </p>
                    {(isCurrent || isCompleted) && step.sub && (
                      <p className={`text-xs mt-0.5 ${isCurrent ? "text-[#E8726A]/70" : "text-gray-400"}`}>
                        {step.sub}
                      </p>
                    )}
                  </div>
        ...
        {(booking.pickup_date || booking.delivery_date) && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <div className={`grid gap-3 ${booking.pickup_date && booking.delivery_date ? "grid-cols-2" : "grid-cols-1"}`}>
              {booking.pickup_date && (
                <div className={`rounded-xl p-3 text-center ${status === "pending" ? "bg-[#E8726A]/10 border border-[#E8726A]/30" : "bg-gray-50"}`}>
                  <p className="text-[10px] font-bold uppercase tracking-wide text-gray-500 mb-1">Pickup</p>
                  <p className={`text-xs font-bold leading-snug ${status === "pending" ? "text-[#E8726A]" : "text-[#0D2240]"}`}>
              ...
              {booking.delivery_date && (
                <div className={`rounded-xl p-3 text-center ${["out_for_delivery","ready","ready_at_warehouse"].includes(status) ? "bg-[#0D2240]/10 border border-[#0D2240]/20" : "bg-gray-50"}`}>
                  <p className="text-[10px] font-bold uppercase tracking-wide text-gray-500 mb-1">Delivery</p>
                  <p className={`text-xs font-bold leading-snug ${["out_for_delivery","ready","ready_at_warehouse"].includes(status) ? "text-[#0D2240]" : "text-[#0D2240]"}`}>
              ...
              {bags.map(bag => {
                ...
                return (
                  <div key={bag.id} className={`rounded-xl border-2 p-3 text-center
                    ...
                    : bagStep === currentStep ? "border-[#E8726A] bg-orange-50"
                    : "border-gray-100 bg-gray-50"}`}>
                    <p className={`font-black text-lg font-mono
                      ${done ? "text-green-600" : bagStep === currentStep ? "text-[#E8726A]" : "text-gray-300"}`}>
                      B{bag.bag_number}
                    </p>
                    <p className={`text-[9px] font-bold uppercase tracking-wide mt-0.5
                      ${done ? "text-green-500" : bagStep === currentStep ? "text-[#E8726A]" : "text-gray-300"}`}>
              ...
              {[...publicEvents].reverse().map(event => (
                <div key={event.id} className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    <p className="text-sm font-bold text-[#0D2240] leading-tight">
