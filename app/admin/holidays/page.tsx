...
export default async function HolidaysPage() {
  ...
  const upcoming = exclusions.filter(e => new Date(e.date) >= new Date())
  const past = exclusions.filter(e => new Date(e.date) < new Date())

  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-[#0D2240]">Pickup &amp; Delivery Exclusions</h1>
        <p className="text-sm text-gray-400">Dates blocked from the booking calendar. Customers cannot schedule pickups or deliveries on these days.</p>
      </div>
      ...
      {/* Custom date add */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
        <h2 className="font-extrabold text-[#0D2240] text-sm uppercase tracking-wide mb-4">Add Custom Date</h2>
      ...
      <h2 className="font-extrabold text-[#0D2240] text-sm uppercase tracking-wide mb-3">
        Upcoming Blocked Dates ({upcoming.length})
      </h2>
      <div className="space-y-2 mb-8">
        {upcoming.length === 0 && <p className="text-sm text-gray-400 py-4">No upcoming blocked dates.</p>}
        {upcoming.map((e) => (
          <div key={e.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-3.5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-[#fdf6f3] flex flex-col items-center justify-center shrink-0">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {new Date(e.date + "T12:00:00").toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
                </p>
              </div>
            </div>
            <form action={removeHoliday.bind(null, e.id)}>
              <button type="submit" className="text-xs text-red-400 hover:text-red-600 font-bold border border-red-100 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors uppercase tracking-wide">
                Remove
              </button>
            </form>
          </div>
