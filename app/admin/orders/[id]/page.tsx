    ...
    <div className="min-h-screen bg-[#f7f8fb]">
      <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        {billingCalculated ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="text-center bg-red-50 border border-red-100 rounded-xl p-4">
                <p className="text-xs text-gray-400 mb-1">Facility Cost</p>
                <p className="text-2xl font-extrabold text-red-600">${(facilityCostCents! / 100).toFixed(2)}</p>
                <p className="text-xs text-gray-400">{assignedFacility ? assignedFacility.name : "facility"}</p>
              </div>
              <div className={`text-center rounded-xl p-4 border ${(customerFinalCents! - facilityCostCents!) > 0 ? "bg-blue-50 border-blue-100" : "bg-amber-50 border-amber-200"}`}>
                <p className="text-xs text-gray-400 mb-1">WashFold Margin</p>
                <p className={`text-2xl font-extrabold ${(customerFinalCents! - facilityCostCents!) > 0 ? "text-blue-700" : "text-amber-600"}`}>
                  ${((customerFinalCents! - facilityCostCents!) / 100).toFixed(2)}
                </p>
                <p className="text-xs text-gray-400">{Math.round(((customerFinalCents! - facilityCostCents!) / customerFinalCents!) * 100)}%</p>
              </div>
            </div>
            {preAuthCents && (
              <p className="text-xs text-gray-400 mt-3 text-center">
                Pre-authorized: ${(preAuthCents / 100).toFixed(2)} · Payment: <span className="font-semibold">{booking.payment_status}</span>
              </p>
            )}
          </div>
        ) : booking.service_type === "wash_fold" && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 text-sm text-amber-700">
            <span className="font-bold">⚖️ Billing pending</span> — weight not yet entered.
            Billing calculated once driver or operator records actual weight.
            {preAuthCents && ` Pre-authorized: $${(preAuthCents / 100).toFixed(2)}.`}
          </div>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-6">
              ...
              {assignedFacility ? (
                <div className="space-y-3">
                  <div className="rounded-xl bg-[#f7f8fb] border border-gray-100 p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-bold text-[#0D2240]">{assignedFacility.name}</span>
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${assignedFacility.processing_mode === "partner_attendant" ? "bg-purple-50 text-purple-700 border border-purple-200" : "bg-blue-50 text-blue-700 border border-blue-200"}`}>
                        {assignedFacility.processing_mode === "partner_attendant" ? "Partner" : "Own Operator"}
                      </span>
                    </div>
                    <p className="text-xs text-gray-400">
                      {assignedFacility.rate_per_lb ? `$${assignedFacility.rate_per_lb}/lb · min ${assignedFacility.minimum_lbs ?? 0} lbs` : "No rate configured"}
                    </p>
                    {assignedFacility.processing_mode === "partner_attendant" && assignedFacility.partner_access_code && (
                      <a href={`/partner/${assignedFacility.partner_access_code}`} target="_blank" rel="noreferrer"
                        className="inline-flex items-center gap-1 text-xs text-[#E8726A] font-semibold mt-1.5 hover:underline">
                  ...
                  <details className="group">
                    ...
                    <div className="mt-3">
                      {bagsAtFacility && (
                        <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-xs text-red-700 font-semibold mb-3">
                          ⚠️ Bags already at {assignedFacility.name}. Physical transfer required.
                        </div>
                      )}
                      <form action={transferFacility} className="flex flex-col gap-2">
                        <input type="hidden" name="bookingId" value={booking.id} />
                        <input type="hidden" name="requiresPhysicalTransfer" value={String(bagsAtFacility)} />
                        <select name="newFacilityId" required className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm text-[#0D2240] focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30">
                          <option value="">— select new facility —</option>
                          {allFacilities?.filter(f => f.id !== assignedFacility.id).map(f => (
                            <option key={f.id} value={f.id}>
                              {f.name} · {f.processing_mode === "partner_attendant" ? "Partner" : "Own Op"}
                        ...
                        <button type="submit" className={`w-full rounded-xl font-bold text-xs py-2.5 text-white transition-colors ${bagsAtFacility ? "bg-red-500 hover:bg-red-600" : "bg-[#0D2240] hover:bg-[#1a3a5c]"}`}>
                          {bagsAtFacility ? "⚠️ Transfer (Physical Move Required)" : "Transfer Facility"}
                        </button>
                      </form>
                    </div>
                ...
                <div className="space-y-3">
                  ...
                  <form action={assignFacility} className="flex flex-col gap-2">
                    <input type="hidden" name="bookingId" value={booking.id} />
                    <select name="facilityId" required className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm text-[#0D2240] focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30">
                      <option value="">— select facility —</option>
                      {allFacilities?.map(f => {
                        const hasMarginWarning = facilityMinForWarning(f.id)
                        return (
                    ...
                    <button type="submit" className="w-full rounded-xl bg-[#E8726A] hover:bg-[#d45f57] text-white font-bold text-sm py-2.5 transition-colors">
                      Assign Facility
                    </button>
                  </form>
                </div>
            ...
            {/* Bags */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <h2 className="font-bold text-[#0D2240] mb-4 text-sm uppercase tracking-wide">Bags ({bags?.length ?? 0})</h2>
              <div className="space-y-3">
                {bags?.map((bag) => (
                  <div key={bag.id} className="flex items-center gap-3 p-3 rounded-xl bg-[#f7f8fb] border border-gray-100">
                    <div className="w-10 h-10 rounded-xl bg-[#0D2240] flex items-center justify-center shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-[#0D2240] text-sm font-mono">{bag.label_code}</p>
                      <p className="text-[10px] text-gray-400 uppercase tracking-wide mt-0.5">{bag.status?.replace(/_/g, " ")}</p>
                    </div>
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide shrink-0 ${STATUS_COLORS[bag.status] ?? "bg-gray-100 text-gray-500"}`}>
                      {bag.status?.replace(/_/g, " ")}
                    </span>
                  </div>
                ))}
        ...
        <div className="mt-6 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-[#0D2240] text-sm uppercase tracking-wide">🚚 Dispatch</h2>
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${shipdaySynced ? "bg-green-50 text-green-700 border border-green-200" : "bg-amber-50 text-amber-700 border border-amber-200"}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${shipdaySynced ? "bg-green-500" : "bg-amber-400"}`} />
              {shipdaySynced ? "Synced with Shipday" : "Not yet in Shipday"}
            </div>
          </div>
          ...
          {!shipdaySynced && (
            <p className="text-sm text-gray-400 mb-5">
              This order predates Shipday ID tracking. New orders store IDs automatically.
            </p>
          )}
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-xl border border-gray-100 bg-[#f7f8fb] p-4">
              ...
              <form action={reschedulePickupAction} className="space-y-2">
                <input type="hidden" name="bookingId" value={booking.id} />
                <input type="date" name="newDate" defaultValue={booking.pickup_date} required
                  className="w-full rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs text-[#0D2240] focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30" />
                <select name="newWindow" defaultValue={booking.pickup_time_window} required
                  className="w-full rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs text-[#0D2240] focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30">
                  <option value="9am-1pm">9am – 1pm</option>
                  <option value="3pm-7pm">3pm – 7pm</option>
                </select>
                <button type="submit" className="w-full rounded-lg bg-[#0D2240] hover:bg-[#1a3a5c] text-white font-bold text-xs py-2 transition-colors">
                  {shipdaySynced ? "Update Pickup → Shipday" : "Update Pickup"}
                </button>
              </form>
            </div>
            ...
            <div className="rounded-xl border border-gray-100 bg-[#f7f8fb] p-4">
              ...
              <form action={rescheduleDeliveryAction} className="space-y-2">
                <input type="hidden" name="bookingId" value={booking.id} />
                <input type="date" name="newDate" defaultValue={booking.delivery_date} required
                  className="w-full rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs text-[#0D2240] focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30" />
                <select name="newWindow" defaultValue={booking.delivery_time_window} required
                  className="w-full rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs text-[#0D2240] focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30">
                  <option value="9am-1pm">9am – 1pm</option>
                  <option value="3pm-7pm">3pm – 7pm</option>
                </select>
            ...
            <div className="rounded-xl border border-gray-100 bg-[#f7f8fb] p-4">
              <p className="text-xs font-bold text-[#0D2240] uppercase tracking-wide mb-3">👤 Assign Driver</p>
              <p className="text-xs text-gray-400 mb-3">Driver&apos;s email as registered in Shipday.</p>
              <form action={assignDriverAction} className="space-y-2">
                <input type="hidden" name="bookingId" value={booking.id} />
                <input type="email" name="driverEmail" placeholder="driver@example.com" required
                  className="w-full rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs text-[#0D2240] placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-[#E8726A]/30" />
                <button type="submit" disabled={!shipdaySynced}
                  className="w-full rounded-lg bg-[#E8726A] hover:bg-[#d45f57] disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-xs py-2 transition-colors">
                  Assign in Shipday
                </button>
                {!shipdaySynced && <p className="text-[10px] text-gray-400 text-center">No Shipday order to assign to yet</p>}
              </form>
              {shipdaySynced && (
                <details className="mt-4 group">
                  <summary className="cursor-pointer text-[10px] text-red-400 hover:text-red-600 font-semibold transition-colors list-none">
                  ...
                  <form action={cancelShipdayAction} className="mt-2">
                    <input type="hidden" name="bookingId" value={booking.id} />
                    <button type="submit" className="w-full rounded-lg bg-red-500 hover:bg-red-600 text-white font-bold text-xs py-2 transition-colors">
                      ❌ Cancel Both Shipday Orders
                    </button>
                    <p className="text-[10px] text-gray-400 mt-1 text-center">Removes them from driver route queues</p>
                  </form>
                </details>
              )}
            </div>
                ...
                <div key={event.id} className="flex gap-4">
                  ...
                  <div className="pb-4 flex-1">
                    ...
                    {event.photo_url && (
                      <a href={event.photo_url} target="_blank" rel="noreferrer" className="block mt-2">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={event.photo_url} alt="Event photo"
                          className="rounded-xl border border-gray-100 max-h-48 max-w-xs object-cover hover:opacity-90 transition-opacity" />
                      </a>
                    )}
                    {event.notes && !event.photo_url && <p className="text-sm text-gray-500 mt-0.5">{event.notes}</p>}
