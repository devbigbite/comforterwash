...
const LIVE_DEMO_URL = "https://www.comforterwash.com"
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#333">
      ...
      <div style="background:#0D2240;border-radius:16px;padding:24px;text-align:center;margin:24px 0">
        <p style="color:rgba(255,255,255,0.6);font-size:11px;letter-spacing:2px;text-transform:uppercase;margin:0 0 10px">Live Demo Site</p>
        <a href="${LIVE_DEMO_URL}" style="display:inline-block;background:#E8726A;color:white;font-weight:800;font-size:14px;text-decoration:none;padding:12px 28px;border-radius:999px;text-transform:uppercase;letter-spacing:0.5px">
          Open the Demo →
        </a>
        <p style="color:rgba(255,255,255,0.4);font-size:12px;margin:14px 0 0">${LIVE_DEMO_URL}</p>
      </div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:20px">
        <tr>
          ...
          <td style="padding:10px 12px;background:#f8f8f8">This is a real, live business (WashFold Orlando) running entirely on the WashFoldClean platform — not a mockup.</td>
        </tr>
        <tr>
          <td style="padding:10px 12px;border-top:1px solid #eee;font-weight:700;color:#0D2240;vertical-align:top">2</td>
          <td style="padding:10px 12px;border-top:1px solid #eee">Browse the site like a customer would: check out "Pick Up &amp; Delivery," "How It Works," "Pricing," "Service Areas," and "FAQ" in the top menu.</td>
        </tr>
        <tr>
          ...
          <td style="padding:10px 12px;border-top:1px solid #eee;background:#f8f8f8">Click "Schedule Pickup" to try the actual booking flow — pick dates, add-ons, and see pricing update live. Feel free to stop before entering real payment details; it's the same flow your customers would use.</td>
        </tr>
        <tr>
          <td style="padding:10px 12px;border-top:1px solid #eee;font-weight:700;color:#0D2240;vertical-align:top">4</td>
          <td style="padding:10px 12px;border-top:1px solid #eee">Bilingual by default — try the EN/ES toggle in the top bar.</td>
        </tr>
        <tr>
          ...
          <td style="padding:10px 12px;border-top:1px solid #eee;background:#f8f8f8">
            Want the full feature list and pricing? Visit <a href="${PLATFORM_URL}" style="color:#E8726A">${PLATFORM_URL}</a> —
            it covers everything from the admin dashboard to driver dispatch to your own branded site.
          </td>
        </tr>
      </table>
...
// Never throws — a failure here shouldn't block the request itself.
export async function autoSendDemoGuideEmail(requestId: string, params: { name: string; email: string; business?: string | null }) {
  try {
    const result = await sendPlatformDemoGuideEmail(params)
    if (!("error" in result)) {
...
// Manual (re)send from /super-admin/demo-requests — e.g. for requests
// submitted before this feature existed, or to follow up again.
export async function resendDemoGuideEmail(requestId: string) {
  ...
  const result = await sendPlatformDemoGuideEmail({
    name: reqRow.name,
    email: reqRow.email,
