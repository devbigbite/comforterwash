...
export async function runRecurringEngine(): Promise<RecurringEngineResult> {
  ...
  const { data: dueAccounts } = await supabase
    ...
    .select("id, location_id, business_name, contact_name, contact_email, contact_phone, address, frequency, pickup_day_of_week, pickup_time_window, delivery_day_of_week, delivery_time_window, num_bags, service_type, next_pickup_date, status, recurring_enabled, stripe_payment_method_id")
    .eq("status", "active")
