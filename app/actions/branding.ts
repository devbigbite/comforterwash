...
export interface DispatchSettings {
  ...
  // Read-only in the UI — generated by the DB default on insert, never
  // user-editable. Exposed here only so the admin page can build and display
  // the tenant's unique webhook URL to paste into Shipday's settings.
  shipday_webhook_secret: string
}
...
export async function getDispatchSettings(): Promise<DispatchSettings> {
  ...
  const { data } = await supabase
    .from("locations")
    .select("shipday_api_key, business_address, business_phone, shipday_webhook_secret")
    .eq("id", locationId)
    .single()
  ...
  return {
    ...
    shipday_webhook_secret: data?.shipday_webhook_secret ?? "",
  }
}
